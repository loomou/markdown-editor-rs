...
pub fn patch_text(tree: &mut BoxTree, doc: &Document, id: NodeId) {
    let kind = doc
        .arena
        .get(id)
        .map(|n| n.kind)
...
pub fn sync_layout(
    ...
    if !changes.is_structural() {
        for c in &changes.changes {
            match c {
                DocChange::TextChanged { node, .. } => patch_text(tree, doc, *node),
...
fn retarget_descendant_slots(tree: &mut BoxTree, doc: &Document, id: NodeId, theme: &LayoutTheme) {
    ...
    for c in kids {
        ...
        retarget_descendant_slots(tree, doc, c, theme);
    }
}
...
fn splice_nodes(
    ...
    for r in removed {
        tree.deferred.remove(&LayoutBoxId::frame(r.index));
        if let Some(id) = existing_box(tree, r.index) {
            drop_subtree(tree, id);
        }
    }
