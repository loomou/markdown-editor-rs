...
fn scrolling_estimated_blocks_paints_at_the_requested_offset(cx: &mut TestAppContext) {
    ...
    assert!(
        drift.is_empty(),
        "the painted position drifted from the request: {drift:?}"
    );
}
...
fn select_all_delete_paints_the_empty_document_at_the_top(cx: &mut TestAppContext) {
    ...
    assert_eq!(
        scroll, 0.0,
        "the empty document must paint at {scroll} scroll"
    );
    assert_eq!(
        state_scroll, 0.0,
        "the scroll must not sit outside the document: {state_scroll}"
    );
    assert!(total <= 600.0, "the empty document is {total} tall");
    assert!(caret.is_some(), "the empty document must paint a caret");
    ...
    assert!(
        x1 > x0,
        "the caret did not move forward after typing: {x0} -> {x1}"
    );
...
fn shrinking_the_document_pulls_the_viewport_back_into_range(cx: &mut TestAppContext) {
    ...
    assert_eq!(
        scroll, 0.0,
        "the viewport must not stay outside the document: {scroll}"
    );
...
fn select_all_does_not_move_the_viewport(cx: &mut TestAppContext) {
    ...
    assert!(
        ...
        "the starting viewport is wrong: {before}"
    );
    ...
    assert_eq!(
        ...
        "select-all changed the scroll: {state_scroll}"
    );
