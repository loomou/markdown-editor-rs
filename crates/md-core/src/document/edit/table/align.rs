use super::arena::{caret_of, cell_at, dims, kids, retarget_header, set_packed, table_packed};
...
use crate::block::{NodeExtra, TableCellAlign, alignment_at, set_alignment};
...
pub(crate) fn set_column_align(
    ...
    let bits = align.bits();
    let packed = table_packed(doc, path.table);
    if alignment_at(packed, path.col) == bits {
    ...
    set_packed(
        doc,
        path.table,
        set_alignment(packed, path.col, bits),
        &mut changes,
    );
